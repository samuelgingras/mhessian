...
"Name: geometric_SS\n"
"Description: univariate geometric count model\n"
...
static void read_data(const mxArray *prhs, Data *data)
{
    ...
    ErrMsgTxt( pr_y != NULL,
    "Invalid input argument::data structure::'y' field missing");
    ErrMsgTxt( mxGetN(pr_y),
    "Invalid input argument:: data structure::column vector expected");
...
static void log_f_y__theta_alpha(double *alpha, Parameter *theta_y, Data *data, double *log_f)
{
    ...
    *log_f = 0.0;
    for(int t=0; t<n; t++) {
        *log_f += data->y[t] * alpha[t] - ( data->y[t] + 1 ) * log( 1 + exp( alpha[t] ) );
    }
}
